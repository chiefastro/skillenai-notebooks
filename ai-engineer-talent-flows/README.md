...
- **New entrants are accelerating** — AI Engineer arrivals steepened every year through 2024 (…80 → 120 → 261), and its *projected* full-2025 (~425–490) clears 2024 comfortably. The adjacent roles (Data Scientist, Data Engineer, ML Engineer) project to roughly their 2024 levels — a plateau.
...
- **AI Engineer** arrivals **accelerated through 2024** (65 → 80 → 120 → 261 across 2021–24) — the steepest climb of the group.
- **2025 is a full-year projection** (the diamonds, with a ± range). Employment records run only to ~Oct 2025, so we scale each role's partial year up by ×1.3–1.5 (see methodology). AI Engineer projects to **~425–490 — clearly above its 2024 (261)**; Data Scientist, Data Engineer, and ML Engineer project to **roughly their 2024 levels — the plateau, not a decline.**
- The projection is deliberately a *range*, not a point: the exact multiplier is uncertain, but the ordering (AI Engineer up, the others flat) is robust across it.
...
- **Arrivals** count role start-events per year (spell-level: consecutive same-role positions merged, so a company change within a role isn't a new arrival; multiple roles at one employer, which LinkedIn nests, are all counted). A start date is a fully-observed past event, so pre-2025 years are observed as-is.
- **2025 projection.** The snapshot's **employment (experience) records are current to ~Oct 2025** (other profile fields extend later), so 2025 is a partial year. The captured *volume* is ~65% of a full year — less than the ~79% you'd get from months elapsed, because the last captured months are thinned by profile-update lag (the 2025-vs-2024 monthly ratio slides from ~0.85 in Q1 to 0.39 in October). We therefore project each role's full 2025 by scaling its observed partial year by **×1.3–1.5** — bounded below by a simple time pro-rata and above by the captured-volume coverage, with the ~×1.4 midpoint accounting for the late-month lag. Shown as **diamonds with a ± range, not observed counts.**
